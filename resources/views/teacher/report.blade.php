    ...
    <div class="space-y-6">
        ...
        @if ($tab === 'summary')
            ...
            <div class="card animate-fade-slide-up stagger-1">
                ...
                <form id="summaryFilterForm" method="GET" action="{{ route('teacher.report') }}">
                        ...
                        <div class="flex flex-col sm:flex-row gap-3">
                            <div class="flex-1">
                                ...
                                <x-expandable-select
                                    ...
                                    onSelect="document.getElementById('summaryFilterForm').submit()"
                                />
                            </div>
                            <div class="flex gap-2 flex-1">
                                <div class="flex-1">
                                    <label class="block text-xs font-semibold uppercase tracking-wider text-bw-400 mb-1">Dari</label>
                                    <input name="summary_start_date" type="date" class="form-input-clean" style="height:38px; border-radius:10px; font-size:13px; padding-top:0; padding-bottom:0;" value="{{ $summaryFilter['start_date'] ?? now()->toDateString() }}" onchange="document.getElementById('summaryFilterForm').submit()">
                                </div>
                                <div class="flex-1">
                                    <label class="block text-xs font-semibold uppercase tracking-wider text-bw-400 mb-1">Sampai</label>
                                    <input name="summary_end_date" type="date" class="form-input-clean" style="height:38px; border-radius:10px; font-size:13px; padding-top:0; padding-bottom:0;" value="{{ $summaryFilter['end_date'] ?? now()->toDateString() }}" onchange="document.getElementById('summaryFilterForm').submit()">
                                </div>
                            </div>
                ...
                <div class="mt-4 flex flex-wrap gap-2">
                    <a href="{{ route('teacher.report.summary.excel', request()->query()) }}" class="btn-secondary gap-2 text-sm">
                    ...
                    <a href="{{ route('teacher.report.summary.pdf', request()->query()) }}" class="btn-secondary gap-2 text-sm">
        ...
        @else
            ...
            <div class="card animate-fade-slide-up stagger-1">
                ...
                <form id="detailFilterForm" method="GET" action="{{ route('teacher.report') }}">
                    ...
                    <div class="filter-panel">
                        <div class="flex flex-col sm:flex-row gap-3">
                            <div class="sm:w-48">
                                ...
                                <x-expandable-select
                                    ...
                                    placeholder="Semua"
                                    onSelect="document.getElementById('detailFilterForm').submit()"
                                />
                            </div>
                            <div class="sm:w-48">
                        ...
                        <div class="flex flex-col sm:flex-row gap-3 mt-3 sm:items-end">
                            <div class="flex gap-2 flex-1">
                                <div class="flex-1">
                                    <label class="block text-xs font-semibold uppercase tracking-wider text-bw-400 mb-1">Dari</label>
                                    <input name="detail_start_date" type="date" class="form-input-clean" style="height:38px; border-radius:10px; font-size:13px; padding-top:0; padding-bottom:0;" value="{{ request('detail_start_date', $startDate->toDateString()) }}" onchange="document.getElementById('detailFilterForm').submit()">
                                </div>
                                <div class="flex-1">
                                    <label class="block text-xs font-semibold uppercase tracking-wider text-bw-400 mb-1">Sampai</label>
                                    <input name="detail_end_date" type="date" class="form-input-clean" style="height:38px; border-radius:10px; font-size:13px; padding-top:0; padding-bottom:0;" value="{{ request('detail_end_date', $endDate->toDateString()) }}" onchange="document.getElementById('detailFilterForm').submit()">
                                </div>
                            </div>
                            <div class="sm:w-auto">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full">
                        <thead><tr class="table-header">
                            ...
                            <th class="text-left py-3 px-4 text-xs font-semibold uppercase tracking-wider">Masuk</th>
                            <th class="text-left py-3 px-4 text-xs font-semibold uppercase tracking-wider">Pulang</th>
                            <th class="text-left py-3 px-4 text-xs font-semibold uppercase tracking-wider">Keterangan</th>
                        </tr></thead>
                        <tbody>
                            @forelse ($rows as $row)
                                <tr class="table-row">
                                    ...
                                    <td class="py-3 px-4 text-sm tabular-nums text-navy-600">{{ $row['Masuk'] }}</td>
                                    <td class="py-3 px-4 text-sm tabular-nums text-navy-600">{{ $row['Pulang'] }}</td>
                                    <td class="py-3 px-4 text-sm text-navy-600 max-w-[200px] truncate">{{ $row['Keterangan Izin'] ?: '-' }}</td>
                                </tr>
                            @empty
                                <tr><td colspan="7" class="py-8 text-center text-bw-400 text-sm">Tidak ada data.</td></tr>
                            @endforelse
