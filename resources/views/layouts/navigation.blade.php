<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
    @php
        $pendingLeaveCount = 0;
        if (auth()->check() && auth()->user()->hasRole('petugas_piket')) {
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    @if (auth()->user()->hasAnyRole(['guru', 'guru_walikelas', 'petugas_piket']) && ! auth()->user()->hasRole('admin'))
                        ...
                        @if (auth()->user()->hasAnyRole(['guru_walikelas', 'petugas_piket']))
                            <x-nav-link :href="route('teacher.report')" :active="request()->routeIs('teacher.report*')">
                                {{ __('Rekap Absensi') }}
                            </x-nav-link>
                        @endif
                    @endif
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @if (auth()->user()->hasAnyRole(['guru', 'guru_walikelas', 'petugas_piket']) && ! auth()->user()->hasRole('admin'))
                ...
                @if (auth()->user()->hasAnyRole(['guru_walikelas', 'petugas_piket']))
                    <x-responsive-nav-link :href="route('teacher.report')" :active="request()->routeIs('teacher.report*')">
                        {{ __('Rekap Absensi') }}
                    </x-responsive-nav-link>
                @endif
            @endif
